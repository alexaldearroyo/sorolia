...
<section class="rounded-xl border border-zinc-200/80 bg-white p-5 shadow-sm">
  <div class="flex items-center justify-end gap-2">
    <button
      type="button"
      class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-zinc-200 bg-white hover:bg-zinc-50"
      onclick={prevMonth}
    ...
    <span class="min-w-[10rem] text-center text-sm font-bold text-zinc-900">{monthTitle}</span>
    <button
      type="button"
      class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-zinc-200 bg-white hover:bg-zinc-50"
      onclick={nextMonth}
  ...
  <div class="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
    <div class="overflow-x-auto">
      <div class="grid grid-cols-7 gap-px rounded-lg border border-zinc-200 bg-zinc-200 text-center text-[11px] font-bold uppercase tracking-wide text-zinc-500">
        {#each ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'] as wd}
          <div class="bg-zinc-50 py-2">{wd}</div>
        {/each}
      </div>
      <div class="grid grid-cols-7 gap-px rounded-b-lg border border-t-0 border-zinc-200 bg-zinc-200">
        {#each grid as cell}
          ...
          >
            ...
            <div class="mt-1 flex flex-wrap gap-0.5">
              ...
              {#if dayEvents.length > 4}
                <span class="text-[9px] font-bold text-zinc-400">+{dayEvents.length - 4}</span>
              {/if}
            </div>
          </button>
        {/each}
    ...
    <aside class="rounded-xl border border-zinc-200 bg-zinc-50/60 p-4">
      {#if selectedDay}
        <h3 class="text-sm font-bold text-zinc-900">{formatIsoToDe(selectedDay)}</h3>
        <p class="mt-1 text-xs text-zinc-500">{selectedEvents.length} event{selectedEvents.length === 1 ? '' : 's'}</p>
        <ul class="mt-3 max-h-[min(60vh,28rem)] space-y-2 overflow-y-auto">
          {#each selectedEvents as ev}
            <li>
              <button
                type="button"
                class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-left text-sm transition hover:border-leah-300 hover:bg-white"
                onclick={() => openEvent(ev)}
              >
                <span class="block text-xs font-semibold uppercase tracking-wide text-zinc-400"
                  >{ev.kind.replaceAll('-', ' ')}</span
                >
                <span class="mt-0.5 block font-medium text-zinc-900">{ev.title}</span>
                {#if ev.amount != null}
                  <span class="mt-0.5 block text-xs tabular-nums text-zinc-600">{currency(ev.amount)}</span>
                {/if}
              </button>
            </li>
          {:else}
            <li class="text-sm text-zinc-500">Nothing scheduled — pick another day.</li>
          {/each}
        </ul>
      {:else}
        <p class="text-sm text-zinc-600">Select a day to see linked invoices, expenses and project reviews.</p>
      {/if}
    </aside>
  </div>
</section>
