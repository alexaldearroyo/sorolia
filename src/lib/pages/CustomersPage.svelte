<script>
  ...
  let { customers, invoices, projects, currency, onUpsertCustomer, onDeleteCustomer, onOpenInvoices, onOpenProjects, onOpenInventory } =
    $props();
  ...
  function deleteCustomer(id) {
    if (onDeleteCustomer(id)) closeEditor();
  }
...
<section class="rounded-xl border border-zinc-200/80 bg-white p-5 shadow-sm">
  <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
    <div>
      <h2 class="text-xl font-bold text-zinc-900">Customers</h2>
      <p class="mt-1 text-sm text-zinc-500">
        Master directory · AR and projects roll up here · {customers.length} accounts
      </p>
    </div>
    <button
      type="button"
